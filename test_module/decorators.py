...
def opt_with_identifier():
    loggerplusplus.opt(identifier="OPT_ONE", colors=True).info(
        "Using opt() with identifier"
    )


def opt_with_bound_logger():
    bound = loggerplusplus.bind(identifier="OPT_BOUND")
    loggerplusplus.opt(logger=bound, depth=1).warning(
        "Using opt() with pre-bound logger"
    )


# ---- LOG_TIMING -----------------------------------------------------------
@loggerplusplus.log_timing(
    identifier="TIMER_ENTER",
    enter_message="Entering {func}...",
    exit_message="Done {func} in {duration:.2f}s",
    show_enter=True,
)
def slow_a():
    time.sleep(0.2)
...
class Service(LoggerClass):
    @loggerplusplus.log_timing(
        identifier="SERVICE_TIMING", exit_message="Service.run took {duration:.2f}s"
    )
